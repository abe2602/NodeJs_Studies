...
const creatUserToken = (userId) =>{
    return jwt.sign({id:userId}, config.jwt_pass, {expiresIn: config.token_time});
}
...
user.post("/create", async(req, res) => {
    ...
    if(!password || !email){
        ...
        return res.status(400).send({erro: "Preencha corretamente!"});
    }
    ...
    try{
        ...
        if(await Users.findOne({email})){
            ...
            return res.status(201).send({usuario: user, token: creatUserToken(user.id)});
        }
    }catch(err){
        return res.status(500).send({error: "Deu o erro "});
    }
});
...
user.post("/auth", async(req, res) =>{
    ...
    try {
        ...
        if(!user){
            return res.status(400).send({erro: "Usuario nao registrado!"});
        }else{
        ...
        return res.status(500).send({erro: "Erro ao buscar!" + err});
    }
});
